...
export const tokenHolders = tableBuilder(
  ...
  {
    id: text("id").primaryKey().notNull(),
    ip_id: text("ip_id").notNull().references(() => ips.id),
...
export const transactions = tableBuilder(
  "transactions",
  {
    ...
    type: text("type", { enum: ["BUY", "SELL", "BURN_SHARE", "CREATOR_FORFEIT"] })
      .notNull(),
...
export const liquidityEvents = tableBuilder(
  "liquidity_events",
  {
    ...
    total_liquidity_distributed: integer("total_liquidity_distributed")
      .notNull()
      .default(0), // USD cents
